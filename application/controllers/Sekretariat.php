    ...
    /* ================================
    TAMPILKAN SURAT PENGAJUAN DALAM MODAL (DENGAN TOOLBAR) - DENGAN LOGIKA JUMLAH DOSEN
    ================================= */
    ...
    /* ================================
    CETAK PDF UNTUK SEKRETARIAT - DENGAN LOGIKA JUMLAH DOSEN (DENGAN QR)
    ================================= */
    public function cetak($id)
    {
        ...
        if ($jumlah_dosen == 1) {
            $view_file = 'surat_satu';  // Tambah folder sekretariat
        } elseif ($jumlah_dosen <= 5) {
            $view_file = 'surat_print';       // Tambah folder sekretariat
        } else {
            $view_file = 'surat_banyak'; // Tambah folder sekretariat
        }
        ...
        $qr_base64 = '';
        try {
            // Perbaikan untuk PHP 8.2.12
            if (extension_loaded('gd')) {
                // Gunakan method yang sesuai dengan library Endroid QR Code yang ada
                if (class_exists('Endroid\QrCode\QrCode')) {
                    $qrCode = new QrCode($validation_url);
                    $qrCode->setSize(160);
                    
                    // Untuk PHP 8.2.12, gunakan writer yang benar
                    if (class_exists('Endroid\QrCode\Writer\PngWriter')) {
                        $writer = new PngWriter();
                        $result = $writer->write($qrCode);
                        
                        // Dapatkan string base64 dari QR Code
                        $qr_base64 = base64_encode($result->getString());
                    } else {
                        // Fallback ke method lama jika writer tidak ada
                        $qr_base64 = base64_encode($qrCode->writeString());
                    }
                } else {
                    // Fallback: buat QR sederhana jika library tidak tersedia
                    $qr_base64 = $this->generate_simple_qr($validation_url);
                }
            }
        } catch (Exception $e) {
            log_message('error', 'QR Code generation failed: ' . $e->getMessage());
            // Fallback ke QR sederhana
            $qr_base64 = $this->generate_simple_qr($validation_url);
        }
        ...
        $options->set('chroot', FCPATH); // Tambahkan chroot untuk akses file lokal
        ...
        try {
            ...
            $filename = "Surat_Tugas_" . $nama_kegiatan_clean . ".pdf";
            ...
            echo "<script>alert('Gagal membuat PDF: " . addslashes($e->getMessage()) . "'); window.close();</script>";
        }
    }
    /* ================================
    ...
    public function set_disposisi()
{
    ...
    // Jika disposisi adalah "Batal", ubah status menjadi "pengajuan"
    if ($disposisi === "Batal") {
        $status_baru = "pengajuan";
        // Reset approval status dan catatan penolakan
        $this->db->where("id", $id);
        $this->db->update("surat", [
            ...
            "status" => $status_baru,
            "approval_status" => json_encode([]), // Reset approval
            "catatan_penolakan" => null // Hapus catatan penolakan lama
        ]);
    } else {
        // Untuk disposisi lain, hanya update disposisi
        $this->db->where("id", $id);
        $this->db->update("surat", [
            "disposisi_status" => $disposisi,
            "disposisi_catatan" => $catatan,
