...
<style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    @media (max-width:768px){
        ...
        .reject-modal-actions{flex-direction:column}
        .reject-btn{justify-content:center}
                    ...
                    <tr onclick="showRowDetail(<?= $s->id ?? 0 ?>)" style="cursor: pointer;" class="clickable-row" id="row-<?= $s->id ?? 0 ?>">
                        ...
                        <td data-label="Aksi">
                            <div style="display:flex;gap:6px">
                                ...
                                <!-- Tombol Reject (Merah) -->
                                <button class="btn btn-reject" onclick="event.stopPropagation(); showRejectModalNew(<?= $s->id ?? 0 ?>, '<?= htmlspecialchars($s->nama_kegiatan ?? '') ?>')" title="Tolak">
...
<div id="bulkApproveModal" class="modal" onclick="modalClickOutside(event,'bulkApproveModal')">
    <div class="approve-modal-content" onclick="event.stopPropagation()">
        ...
        <div class="approve-modal-body">
            ...
            <form id="bulkApproveForm" method="POST" action="<?= base_url('kaprodi/process_multi_approve') ?>">
                <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
...
<div id="bulkRejectModal" class="modal" onclick="modalClickOutside(event,'bulkRejectModal')">
    <div class="bulk-reject-modal-content" onclick="event.stopPropagation()">
        ...
        <div class="bulk-reject-body">
            ...
            <form id="bulkRejectForm" method="POST" action="<?= base_url('kaprodi/process_multi_reject') ?>">
                <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
...
<!-- Success Modal (for both single and bulk actions) -->
<div id="successModal" class="modal" onclick="modalClickOutside(event,'successModal')">
    <div class="success-modal-content" onclick="event.stopPropagation()">
        <div class="success-modal-header">
            <h3><i class="fa-solid fa-check-circle"></i> Berhasil</h3>
        ...
        <div class="success-modal-body">
            ...
            <h4 id="successTitle">Pengajuan Berhasil Diproses</h4>
            <p id="successMessage"></p>
            
            <div id="successItems" class="success-items">
                <!-- Daftar item yang berhasil diproses -->
            </div>
            
            <div style="margin-top:20px">
                <button class="approve-btn approve-btn-submit" onclick="closeModal('successModal')">
    ...
    if (items && items.length > 0) {
        items.forEach(item => {
            ...
            itemDiv.innerHTML = `
                ...
                <small style="color:#7f8c8d">${escapeHtml(item.details)}</small>
            `;
            itemsContainer.appendChild(itemDiv);
        });
    }
...
// ============================================
// FUNGSI UTAMA UNTUK TOMBOL DAN KLIK BARIS
// ============================================
...
// Fungsi untuk menampilkan surat pengajuan (tombol mata)
async function showSurat(id) {
    try {
        // Tampilkan loading
...
async function showEvidenModal(suratId) {
    try {
        ...
        // LOGIKA: Jika hanya 1 file, langsung preview
...
function generateDetailContent(item) {
    ...
    // LOGIKA: Tentukan tampilan berdasarkan jenis_date
...
// Fungsi untuk mendapatkan array file eviden dari data
