...
<style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    /* Return Modal Styles - TIDAK DIPERLEBAR */
...
<div class="navbar">
    <h2><i class="fa-solid fa-user-tie"></i> Dashboard Kaprodi - Pengajuan Ditolak</h2>
    <div></div>
</div>
...
class ModalManager {
    ...
    createReturnModal(data) {
        ...
        if (data.suratId) {
            modalContent.querySelector('.return-form').action = '<?= base_url("kaprodi/return_pengajuan/") ?>' + data.suratId;
        }
...
function generateDetailContent(item) {
    ...
    <div class="modal-actions">
        ...
        ${getVal('status') === 'ditolak KK' ? `
        <div style="display:flex;gap:12px;margin-left:auto">
            <button class="modal-btn" style="background: #ff9800;" onclick="showReturnModal(${item.id}, '${escapeHtml(getVal('nama_kegiatan'))}')">
                <i class="fa-solid fa-undo"></i> Kembalikan
            </button>
            <button class="modal-btn" style="background: #28a745;" onclick="handleEvidenClick(${item.id}, '${escapeHtml(getVal('nama_kegiatan'))}')">
