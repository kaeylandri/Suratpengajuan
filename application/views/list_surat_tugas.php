...
<head>
    ...
    /* Table styles - PERBAIKAN RESPONSIF UTAMA */
    ...
    /* Atur lebar kolom dengan persentase responsif */
    ...
    /* Action buttons with icons - PERBAIKAN UTAMA */
    .action-buttons {
        display: flex;
        gap: 6px;
        justify-content: center;
    ...
    @media (max-width: 992px) {
        ...
        /* Atur ulang lebar kolom untuk tablet */
        #tabelSurat th:nth-child(1),
        #tabelSurat td:nth-child(1) {
            width: 5%;
            max-width: 40px;
    ...
    @media (max-width: 768px) {
        ...
        /* Sembunyikan kolom divisi di mobile landscape */
        #tabelSurat th:nth-child(6),
        #tabelSurat td:nth-child(6) {
            display: none;
        }
    ...
    @media (max-width: 576px) {
        ...
        /* Untuk mobile portrait, sembunyikan beberapa kolom */
        #tabelSurat th:nth-child(4),
        #tabelSurat td:nth-child(4) {
            display: none;
        }
    ...
    /* Print styles */
    @media print {
        .navbar, .fik-db-side-menu, .footer, .filter-bar, 
        .filter-builder, .multi-actions, .action-buttons-group,
        .btn-back, .btn-add {
            display: none !important;
        }
    ...
    .btn-edit-disabled {
        pointer-events: auto !important; /* Untuk menampilkan alert */
    }
...
<body>
    ...
    <nav class="navbar fixed-top">
        <div class="container">
            <button class="db-menu-trigger show-mobile" onclick="toggleSidebar()" aria-label="Toggle Menu">
                <i class="fas fa-bars"></i>
            </button>
            <div class="navbar-brand akun">
                <a href="https://ifik.telkomuniversity.ac.id/">
                    <img src="https://ifik.telkomuniversity.ac.id/assets/img/logo/logo-dummy.png" alt="Logo" />
                </a>
            </div>
                ...
                <ul class="right akun">
                    ...
                    <li>
                        <div class="dropdown not-dropdown">
                            <a class="btn btn-icon" id="active" data-bs-toggle="dropdown" href="#" aria-expanded="false">
        ...
        <div id="accordion">
            ...
            <div class="card">
                <a href="<?= base_url('list-surat-tugas') ?>" class="btn active">
                    <i class="fas fa-palette"></i> List Surat Tugas
                </a>
            </div>
            <div class="card">
                <a href="https://ifik.telkomuniversity.ac.id/Pic_kk/input_surat" class="btn">
                    <i class="fas fa-envelope-open-text"></i> Surat Tugas
                </a>
            </div>
            <div class="divider"></div>
            <div class="card">
                <button class="btn" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded="true" aria-controls="collapse2">
            ...
            <div class="card">
                <button class="btn" data-bs-toggle="collapse" data-bs-target="#ticketing" aria-expanded="true" aria-controls="ticketing">
                    <i class="fas fa-door-open"></i> Ticketing
                </button>
    ...
    <div class="main-content">
        ...
        <div class="table-responsive-container">
            <table id="tabelSurat" class="display">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr class="row-detail" data-detail='<?= $data_detail_attr; ?>' data-id="<?= $s->id; ?>">
                        ...
                        <td title="<?= htmlspecialchars($s->nama_kegiatan); ?>">
                        ...
                        <td>
                            <div class="action-buttons">
                                <button class="btn-icon-action btn-status" title="Lihat Status" onclick="event.stopPropagation(); showStatusModal(<?= $s->id; ?>)">
                                ...
                                <?php else: ?>
                                    <button class="btn-icon-action btn-edit btn-edit-disabled" 
                                            ...
                                            onclick="event.stopPropagation(); showEditWarning(this)"
                                ...
                                title="Hapus" 
                                onclick="event.stopPropagation(); return confirm('Hapus data ini?')">
    ...
    document.addEventListener('click', function(e) {
        if (window.innerWidth <= 992) {
            if (!e.target.closest('.fik-db-side-menu') && !e.target.closest('.db-menu-trigger')) {
    ...
    // Function untuk menampilkan warning edit
    function showEditWarning(button) {
        const row = button.closest('tr');
        const status = 'Status: ' + (row.querySelector('td:nth-child(7)')?.textContent || 'Tidak diketahui');
        ...
        fetch('<?= site_url("surat/get_status/") ?>' + suratId)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
                        text: 'Gagal memuat status: ' + (data.message || 'Unknown error'),
                        confirmButtonColor: '#FB8C00'
                    });
                }
            })
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Terjadi kesalahan saat memuat status',
                    confirmButtonColor: '#FB8C00'
                });
            });
    }
    ...
    $(document).ready(function () {
        ...
        $('#btnMultiEdit').click(function(){
            if(selectedIds.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Peringatan',
                    text: 'Pilih minimal 1 item untuk di-edit',
                    confirmButtonColor: '#FB8C00'
                });
                return;
            }
        ...
        $('#btnMultiDelete').click(function(){
            ...
            Swal.fire({
                ...
                html: `Apakah Anda yakin ingin menghapus <strong>${selectedIds.length}</strong> item yang dipilih?`,
                icon: 'warning',
        ...
        function deleteMultipleSurat(ids) {
            ...
            Swal.fire({
                title: 'Menghapus...',
                html: `Sedang menghapus ${total} item...<br><br><div class="progress" style="height: 10px;">
                    <div class="progress-bar progress-bar-striped progress-bar-animated" id="delete-progress" style="width: 0%"></div>
                </div>`,
                ...
                setTimeout(() => {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#delete-progress').css('width', progress + '%');
                            ...
                            if (successCount + errorCount === total) {
                                ...
                                if (successCount > 0) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Berhasil',
                                        html: `${successCount} data berhasil dihapus${errorCount > 0 ? ', ' + errorCount + ' gagal' : ''}`,
                                        confirmButtonColor: '#FB8C00'
                                    });
                                    updateMultiActions();
                                }
                        ...
                        error: function(xhr, status, error) {
                            ...
                            if (successCount + errorCount === total) {
                                ...
                                Swal.fire({
                                    icon: errorCount === total ? 'error' : 'warning',
                                    title: errorCount === total ? 'Gagal' : 'Peringatan',
                                    html: `${successCount} data berhasil dihapus, ${errorCount} gagal`,
                                    confirmButtonColor: '#FB8C00'
                                });
                                updateMultiActions();
                            }
        ...
        $('#tabelSurat tbody').on('click','tr.row-detail',function(e){
            if($(e.target).closest('input, a, button, .nama-dosen-more').length) return;
                ...
                data.eviden.forEach((file, index) => {
                    ...
                    html += `
                        <div class="file-item">
                            ...
                            <div class="file-info" onclick="previewFile('${escapeHtml(fileUrl)}', '${escapeHtml(fileName)}', '${fileType}')" style="cursor: pointer;">
                            ...
                            <div class="file-actions">
                                <a href="${fileUrl}" class="download-btn" download="${escapeHtml(fileName)}" target="_blank" onclick="event.stopPropagation()">
                                    <i class="fas fa-download"></i> Download
                                </a>
                                ${canPreview ? `
                                    <button class="preview-btn" onclick="event.stopPropagation(); previewFile('${escapeHtml(fileUrl)}', '${escapeHtml(fileName)}', '${fileType}')">
        ...
        function adjustTableForMobile() {
            ...
            const table = $('#tabelSurat').DataTable();
            
            if (width <= 768) {
                // Hide columns on mobile
                table.columns([5]).visible(false); // Hide Divisi column
                if (width <= 576) {
                    table.columns([3]).visible(false); // Hide Jenis Pengajuan column
                } else {
